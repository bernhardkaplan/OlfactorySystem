...
#define usetable usetable_lcafixed
 ...
 "mysinf_lcafixed", &mysinf_lcafixed,
	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 3);
 	...
 	ivoc_help("help ?1 lcafixed /home/bernhard/workspace/olfactory_system/OlfactorySystem/neuron_files/x86_64/lcafixed.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
 static double eca = 70;
 static double *_t_mysinf;
...
/*CVODE*/
 ...
 {
   rates ( _threadargscomma_ v ) ;
    s = s + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / stau)))*(- ( ( ( mysinf ) ) / stau ) / ( ( ( ( - 1.0) ) ) / stau ) - s) ;
...
static int _hoc_bet() {
 ...
 return; }
 if (_i >= 200) {
 mysinf = _t_mysinf[200];
 rinf = _t_rinf[200];
 stau = _t_stau[200];
 rtau = _t_rtau[200];
 return; }
 _theta = _xi - (double)_i;
 mysinf = _t_mysinf[_i] + _theta*(_t_mysinf[_i+1] - _t_mysinf[_i]);
...
static _initlists() {
 ...
 _slist1[1] = &(r) - _p;  _dlist1[1] = &(Dr) - _p;
   _t_mysinf = makevector(201*sizeof(double));

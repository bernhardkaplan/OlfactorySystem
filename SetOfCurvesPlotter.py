...
class SetOfCurvesPlotter(object):
    ...
    def plot_set_of_curves(self, output_fn=None, cell_type='orn'):
        ...
        y_avg, y_std = self.get_average_group_curve(x_data, y_data, self.params['rel_orn_mit'])
        ...
        n_orns = len(x_data)
        lw = 1
        for i_ in xrange(n_orns):
            if cell_type == 'orn':
                color_group_idx = i_ / self.params['rel_orn_mit']
